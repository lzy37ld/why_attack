...
do
	...
	do
		...
		# need to change the setting in config
		python evaluate_for_test_prompter.py target_lm=vicuna-chat force_append=true prompt_way=own data_args.split=${split} ppl=${ppl} s_p_t_dir=$s_p_t_dir target_lm.generation_configs.max_new_tokens=100 prompt_own_list_name=llama2_lowest
		python evaluate_for_test_prompter.py target_lm=vicuna-chat force_append=true prompt_way=own data_args.split=${split} ppl=${ppl} s_p_t_dir=$s_p_t_dir target_lm.generation_configs.max_new_tokens=100 prompt_own_list_name=llama2_lowest_at_each_step
		python evaluate_for_test_prompter.py target_lm=mistral-instruct force_append=true prompt_way=own data_args.split=${split} ppl=${ppl} s_p_t_dir=$s_p_t_dir target_lm.generation_configs.max_new_tokens=100 prompt_own_list_name=llama2_lowest
		python evaluate_for_test_prompter.py target_lm=mistral-instruct force_append=true prompt_way=own data_args.split=${split} ppl=${ppl} s_p_t_dir=$s_p_t_dir target_lm.generation_configs.max_new_tokens=100 prompt_own_list_name=llama2_lowest_at_each_step
	done
done
...
do
	...
	do
		...
		# need to change the setting in config
		python evaluate_for_test_prompter.py target_lm=llama2-chat force_append=true prompt_way=own data_args.split=${split} ppl=${ppl} s_p_t_dir=$s_p_t_dir target_lm.generation_configs.max_new_tokens=100 prompt_own_list_name=vicuna_lowest
		python evaluate_for_test_prompter.py target_lm=llama2-chat force_append=true prompt_way=own data_args.split=${split} ppl=${ppl} s_p_t_dir=$s_p_t_dir target_lm.generation_configs.max_new_tokens=100 prompt_own_list_name=vicuna_lowest_at_each_step
		python evaluate_for_test_prompter.py target_lm=mistral-instruct force_append=true prompt_way=own data_args.split=${split} ppl=${ppl} s_p_t_dir=$s_p_t_dir target_lm.generation_configs.max_new_tokens=100 prompt_own_list_name=vicuna_lowest
		python evaluate_for_test_prompter.py target_lm=mistral-instruct force_append=true prompt_way=own data_args.split=${split} ppl=${ppl} s_p_t_dir=$s_p_t_dir target_lm.generation_configs.max_new_tokens=100 prompt_own_list_name=vicuna_lowest_at_each_step
	done
done
